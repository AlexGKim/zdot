...
value \alex{at a single redshift} directly indicates acceleration, with no need to know the dynamics, 
...
\alex{I don't understand this Only when .. do we logic.  $a(t)$ is interesting for cosmologists (LSS) without a need to
reference a model.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{Theoretical Speculations} \label{sec:methods} 
...
% 
\eric{Why must span? Do you want to say that can cut out part of spectrum 
away from doublet to reduce noise?} 
%
...
% 
\eric{Delete?: Although we use its velocity dispersions,} 
% 
...
% 
\eric{Check my $\Delta\tau$ correction.} 
% 
...
\eric{Can delete:   The four phases parameterized by $n$ are defined in Eqn.~\ref{edisig:eqn}.} 
\eric{Add vertical dotted line showing line wavenumber?} 
\label{edicounts:fig}}
\end{figure}
...
Flux miscalibration still affects the line profiles, resulting in potential bias in effect 
\eric{What?} 
...
\eric{Clarify; I thought arc lines were far from science lines} 
...
$x=0$) of five \eric{Check} SHS configurations giving different
   Littrow numbers according to 
   $n=1/3.5, 1/1.05, 1, 1.05, 3.5$  in Eq.~\ref{littrowchoices:eqn}. 
Note the curves are offset; each has approximately 100000 counts. 
\eric{Check} 
   \label{shscounts:fig}}
\end{figure}
...
\begin{figure}[t]
   ...
   \plotone{edshs} 
   \caption{The output signal for positive $x$ (it is symmetric about
$x=0$) of the ED-SHS \eric{Check} configuration with 
    $\sigma_0=(\sigma_1+\sigma_2)/2$. 
Note the curves are offset; each has approximately 40000 \eric{Check} counts. 
\label{edshs:fig}}
\end{figure} 
...
In addition, the flux signals per pixel \eric{Check} are further reduced relative to the SHS, relaxing flux calibration requiirements. 
