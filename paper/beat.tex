...
capable of measuring redshifts to a precision 
of $2\times 10^{-9}$ 
...
\alex{For the sign maybe$-\mbox{sgn}(1/3+w_{tot})$ is better notation.
Of sign(+/-) in the column header and  $w_{\rm tot}(</>)-1/3$
The current notation requires non-standard boolean assignment of False to $-1$
instead of 0.}
\label{tab:drbao}}
\end{table} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\section{Experimental Approaches \alex{For Measuring Changes in Redshift}} \label{sec:tech} 
...
in Eqn.~\ref{input:eqn} as the standard deviation of the first line.
...
and $\Delta \tau$ represents the changes in delay in the series of exposures; for our calculations we choose  $\Delta \tau = n/2(\sigma_1+\sigma_2)$ with $n\in\{0,1,2,3\}$
with the change achieved by adjusting an interferometer arm length by fourths of the inverse average wavenumber. 
\alex{(This choice for the set of  $\Delta \tau$ is made for mathematical convenience and generally does not strongly affect the statistical determination of whirl.)}
...
on a 10-m telescope). \alex{``Gold'' targets, with brighter and narrow doublet
...
spectrographs. 
\alex{It is of interest to test whether the stability and/or calibration precision
possible on these instruments can satisfy the requirements imposed by the science.}
...
We acknowledge helpful discussions with Nao Suzuki, Jeff Newman and Stephen Bailey.  
