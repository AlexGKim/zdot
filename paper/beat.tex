...
The Universe gives us for free a time gap for measuring
...
be fantastic (although as will be discussed shortly, we cannot take advantage of the
Lyman-$\alpha$ absorbers in systems with long time delays and large angular separation).
...
differences that are too small to detect with current methods.
...
of uncertainty.Text from Jerry about how to do a simultaneous observation with fibers with
...
the previous year.  This ``time anchoring'' adds robustness to 
...
Unlike for time-delay distances 
...
the redshifts of each Lyman-$\alpha$ absorber; the redshift drift for each can
be measured 
